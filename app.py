...
def compute_app_result(inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if section_mode == "Desde curvas de nivel v13":
            if not curves:
                raise RuntimeError("Seleccionaste el modo v13, pero no cargaste curvas de nivel de apoyo KMZ/KML/DXF.")
            sections = generate_sections_from_contours_v13(axis_useful, curves, section_spacing, half_width)
            if not sections:
                raise RuntimeError("No se pudieron generar secciones desde curvas. Verifica que las curvas tengan cota Z/nombre de cota y que crucen el tramo útil.")
            ...
            sections = generate_natural_sections(axis_useful, dem, crs_proj, section_spacing, half_width, station_step) if dem is not None else []
        elif section_mode == "Prismática por parámetros":
            ...
            if sections_excel is None:
                warnings.append("Modo Excel seleccionado, pero no se cargó Excel de secciones.")
...
"""
    ...
    with tab5:
        ...
        st.download_button("Descargar KMZ eje + secciones", kmz_bytes, file_name="hidrosed_eje_cauce_secciones_v5_modelo_v13.kmz", mime="application/vnd.google-earth.kmz")
        ...
        summary = {
            "modulo": "HidroSed Eje Cauce y Secciones v5 modelo v13",
        ...
        st.download_button("Descargar JSON técnico actualizado", json.dumps(summary, indent=2, ensure_ascii=False).encode("utf-8"), file_name="resumen_tecnico_eje_secciones_v4.json", mime="application/json")


# --------------------------------------------------------------------------------------
# App UI
# --------------------------------------------------------------------------------------
st.title("🌊 HidroSed · Módulo Eje Cauce y Secciones v4")
st.caption("Eje útil obligatorio · DEM OpenTopography/manual · modo v13 de curvas · visor sección por km · perfil 3D hidráulico progresiva-offset-cota")
...
sections_excel = st.sidebar.file_uploader("Excel de secciones prismáticas opcional", type=["xlsx", "xls"])
...
section_mode = st.sidebar.selectbox(
    ...
    ["Desde curvas de nivel v13", "Desde DEM natural", "Prismática por parámetros", "Desde Excel prismático"],
    help="El modo v13 replica el modelo probado: secciones por intersección con curvas de nivel y perfil 3D en progresiva-offset-cota."
)
...
if section_mode == "Desde curvas de nivel v13":
    if curves_file is None:
        st.sidebar.warning("Para usar el modo v13 debes cargar curvas de nivel de apoyo KMZ/KML/DXF con cotas.")
elif section_mode == "Desde DEM natural":
